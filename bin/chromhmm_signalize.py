...
@arg('--chromsizes_file', help='Path to a 2-column chrom sizes file '
                               '(chrom\\tsize); overrides pybedtools.chromsizes')
...
def rebin(configfile, genome, binsize=200, quiet=False, binned_dir='binned', chromsizes_file=None):
    """
    ...
    if chromsizes_file:
        logger.info('use provided chromfile: %s' % (chromsizes_file))
        genome = {}
        with open(chromsizes_file) as fh:
            for line in fh:
    ...
    for celltype, mark, path, control in config.config:
        ...
        # ---- CHANGED: convert bigWig to a per-run unique bedGraph under binned_dir
        # if is_bigwig(path):
        #     bg = path + '.bedgraph'
        ...
        if is_bigwig(path):
            # unique, deterministic name per (input path, binsize)
            base = os.path.basename(path)
            bg = os.path.join(
                binned_dir,
                '{}.binsize-{}.bedgraph'.format(base, int(binsize))
            )
            if not os.path.exists(bg):
                logger.info('converting to bedgraph: %s -> %s' % (path, bg))
                # UCSC tool must be on PATH
                os.system('bigWigToBedGraph %s %s' % (path, bg))
            else:
                logger.info('%s already exists, using it' % bg)
        else:
            # if a bedGraph was provided, copy into binned_dir with unique name
            # to avoid cross-run collisions; cheap link/copy
            src_bg = path
            base = os.path.basename(src_bg)
            bg = os.path.join(binned_dir, '{}.binsize-{}.bedgraph'.format(base, int(binsize)))
            if not os.path.exists(bg):
                logger.info('copying bedGraph into run dir: %s -> %s' % (src_bg, bg))
                # use a link if possible, fallback to copy
                try:
                    os.link(src_bg, bg)
                except Exception:
                    import shutil
                    shutil.copyfile(src_bg, bg)
            else:
                logger.info('%s already exists, using it' % bg)
